...
from tensorflow.python.keras.layers import Conv2D, Dense, AvgPool2D, Flatten, Dropout, MaxPool2D
...
def train_model(train_data, train_labels, test_data, test_labels, epochs):
  ...
  my_model = Sequential([
    Conv2D(filters=32, kernel_size=8, activation="relu", input_shape=input_shape, padding="same"),
    MaxPool2D(padding="same"),
    Conv2D(filters=64, kernel_size=8, activation="relu", padding="same"),
    MaxPool2D(padding="same"),
    Dropout(0.25),
    Flatten(),
    Dropout(0.5),
...
class App:
  ...
  def train_model(self, epochs):
    train_data, train_labels = prepare_data(*load_data(60000, "./data/train-images-idx3-ubyte", "./data/train-labels-idx1-ubyte"))
    test_data,   test_labels = prepare_data(*load_data(10000, "./data/train-images-idx3-ubyte", "./data/train-labels-idx1-ubyte"))
  ...
  def show_predictions(self, n):
    ...
    probs = self.model.predict(prep_data)
